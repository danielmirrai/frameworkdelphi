...
uses
  ...
  DateUtils, XMLDoc, ShellAPI, Controls, Variants, Classes;
...
type
  ...
  public
    ...
    class procedure CopyDFMForLib(const psDirectoryOrigin, psDirectoryDestination: String);
    ...
    class procedure MyException(Sender: TObject; E: Exception); overload;
    ...
    class function CompareIgnoreCase(const psText, psTextCompare: string; const pbRequereValue: Boolean = False): Boolean;
...
class function TDMUtils.CompareIgnoreCase(const psText, psTextCompare: string; const pbRequereValue: Boolean): Boolean;
begin
  Result := (Trim(UpperCase(psText)) = Trim(UpperCase(psTextCompare))) and ((not pbRequereValue) or (pbRequereValue and TDMUtils.ExistValue(psText)));
end;

class procedure TDMUtils.CopyDFMForLib(const psDirectoryOrigin, psDirectoryDestination: String);
var
  poListFiles: TStringList;
  nIndex: Integer;
  sFile: String;
begin
  if not (DirectoryExists(psDirectoryOrigin) and DirectoryExists(psDirectoryDestination)) then
    Exit;
  ...
  try
    ...
    begin
      ...
      if (TDMUtils.FileExists2(sFile) and
        not TDMUtils.CompareIgnoreCase(ExtractFilePath(sFile), ExtractFilePath(psDirectoryDestination))) then
        CopyFile(Pchar(poListFiles.Strings[nIndex]), Pchar(ExtractFilePath(psDirectoryDestination) + ExtractFileName(sFile)), False);
    end;
  finally
    TDMUtils.DestroyObject(poListFiles);
  end;
...
class procedure TDMUtils.MyException(Sender: TObject; E: Exception);
begin
  MyException(Sender);
end;
