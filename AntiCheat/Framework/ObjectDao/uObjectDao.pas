...
type
  ...
  private
    ...
    FCanApplyDataDB: Boolean;
    procedure SetCanApplyDataDB(const Value: Boolean);
  ...
  public
    ...
    property CanApplyDataDB: Boolean read FCanApplyDataDB write SetCanApplyDataDB;
  end;
...
begin
  ...
  try
    if CanApplyDataDB and TDMUtils.FileExists2(sFile) then
...
begin
  ...
  if not (CanApplyDataDB and TDMUtilsDao.IsActiveDataSet(CDSDados)) then
    Exit;
...
procedure TObjectDao.SetCanApplyDataDB(const Value: Boolean);
begin
  FCanApplyDataDB := Value;
end;
