...
begin
  Application.MessageBox(PChar(psMessage), PChar(sCST_NameProject + sCST_TraceSpaces + sCST_Alert), mb_ok + MB_ICONWARNING);
end;

class procedure TDMUtilsMessage.ShowMessageError(const psMessage: string);
begin
  Application.MessageBox(PChar(psMessage), PChar(sCST_NameProject + sCST_TraceSpaces + sCST_Error), mb_ok + MB_ICONERROR);
end;

class procedure TDMUtilsMessage.ShowMessageOk(const psMessage: string);
begin
  Application.MessageBox(PChar(psMessage), PChar(sCST_NameProject + sCST_TraceSpaces + sCST_Information), mb_ok + MB_ICONINFORMATION);
end;
