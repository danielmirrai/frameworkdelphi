...
type
  ...
  protected
    ...
    function GetMD5: string;
        procedure SetListDescriptionFileInfo(const poTranslate: Pointer;
      const poBuffer: PChar; var pnLength: Integer);
    ...
    procedure SetDescriptionInfoFile(const pnIndex: Integer;
      const psDescription: string);
  public
    ...
    property OriginalFilename: string read GetOriginalFilename
      write SetOriginalFilename;
    property ProductName: string read GetProductName write SetProductName;
    property ProductVersion: string read GetProductVersion
      write SetProductVersion;
  ...
  public
    ...
    property Items[Index: Integer]: TDMFileInfoUtils read GetItems
      write SetItems;
  end;
...
begin
  ...
  Result := TDMUtils.IIf((poFileFlags = VS_FF_DEBUG),
    Concat(Result, '*Debug* '), Result);
  Result := TDMUtils.IIf((poFileFlags = VS_FF_SPECIALBUILD),
    Concat(Result, '*Special Build* '), Result);
  Result := TDMUtils.IIf((poFileFlags = VS_FF_PRIVATEBUILD),
    Concat(Result, '*Private Build* '), Result);
  Result := TDMUtils.IIf((poFileFlags = VS_FF_PRERELEASE),
    Concat(Result, '*Pre-Release Build* '), Result);
  Result := TDMUtils.IIf((poFileFlags = VS_FF_PATCHED),
    Concat(Result, '*Patched* '), Result);
  FAttribute := Result;
end;
...
function TDMFileInfoUtils.GetSize: Double;
begin
  Result := FSize;
end;
...
begin
  ...
  case poFileOS of
    VOS_UNKNOWN:
      Result := Concat(Result, 'Desconhecido');
    VOS_DOS:
      Result := Concat(Result, 'MS-DOS');
    VOS_OS216:
      Result := Concat(Result, '16-bit OS/2');
    VOS_OS232:
      Result := Concat(Result, '32-bit OS/2');
    VOS_NT:
      Result := Concat(Result, 'Windows NT');
    VOS_NT_WINDOWS32, 4:
      Result := Concat(Result, 'Win32 API');
    VOS_DOS_WINDOWS16:
      Result := Concat(Result, '16-bit Windows ', 'sob MS-DOS');
  else
    Result := Concat(Result, 'Fora do Padrão. Código: ', IntToStr(poFileOS));
  end;
end;

function TDMFileInfoUtils.GetTypeArq(const poFileType, poFileSubtype
  : DWord): string;
begin
  Result := sCST_EmptyStr;
  case poFileType of
    VFT_UNKNOWN:
      Result := Concat(Result, 'Desconhecido');
    VFT_APP:
      Result := Concat(Result, 'Aplicacao');
    VFT_DLL:
      Result := Concat(Result, 'Dynamic-Link Lib.');
    VFT_DRV:
      begin
        ...
        case poFileSubtype of
          VFT2_UNKNOWN:
            Result := Concat(Result, 'Desconhecido');
          VFT2_DRV_PRINTER:
            Result := Concat(Result, 'de Impressao');
          VFT2_DRV_KEYBOARD:
            Result := Concat(Result, 'de Teclado');
          VFT2_DRV_LANGUAGE:
            Result := Concat(Result, 'de Idioma');
          VFT2_DRV_DISPLAY:
            Result := Concat(Result, 'de Vídeo');
          VFT2_DRV_MOUSE:
            Result := Concat(Result, 'de Mouse');
          VFT2_DRV_NETWORK:
            Result := Concat(Result, 'de Rede');
          VFT2_DRV_SYSTEM:
            Result := Concat(Result, 'de Sistema');
          VFT2_DRV_INSTALLABLE:
            Result := Concat(Result, 'Instalavel');
          VFT2_DRV_SOUND:
            Result := Concat(Result, 'Multimida');
        end;
      end;
    VFT_FONT:
      begin
        ...
        case poFileSubtype of
          VFT2_UNKNOWN:
            Result := Concat(Result, 'Desconhecida');
          VFT2_FONT_RASTER:
            Result := Concat(Result, 'Raster');
          VFT2_FONT_VECTOR:
            Result := Concat(Result, 'Vetorial');
          VFT2_FONT_TRUETYPE:
            Result := Concat(Result, 'TrueType');
        end;
      end;
    VFT_VXD:
      Result := Concat(Result, 'Virtual Device');
    VFT_STATIC_LIB:
      Result := Concat(Result, 'Static-Link Lib.');
  end;
  FTypeFile := Result;
end;
...
procedure TDMFileInfoUtils.SetDescriptionInfoFile(const pnIndex: Integer;
  const psDescription: string);
begin
  case pnIndex of
    1:
      FCompanyName := psDescription;
    2:
      FFileDescription := psDescription;
    3:
      FFileVersion := psDescription;
    4:
      FInternalName := psDescription;
    5:
      FLegalCopyright := psDescription;
    6:
      FOriginalFilename := psDescription;
    7:
      FProductName := psDescription;
    8:
      FProductVersion := psDescription;
    9:
      FComments := psDescription;
  end;
end;
...
begin
  ...
  Size := TDMUtils.SizeFile(aFileName);
  GetMem(vBuffer, Succ(vBufferSize));
  try
    if not(GetFileVersionInfo(PChar(aFileName), nCST_Zero, vBufferSize, vBuffer)
      and VerQueryValue(vBuffer, '\VarFileInfo\Translation', vTranslate,
      UINT(vLen))) then
      exit;
    SetListDescriptionFileInfo(vTranslate, vBuffer, vLen);

    if not VerQueryValue(vBuffer, PChar(sCST_EmptyStr), vFixedFileInfo,
      UINT(vLen)) then
      exit;
...
procedure TDMFileInfoUtils.SetListDescriptionFileInfo(const poTranslate
  : Pointer; const poBuffer: PChar; var pnLength: Integer);
var
  nIndex: Integer;
  nStrFileInfo: string;
...
begin
  ...
  begin
    nStrFileInfo := Format('\StringFileInfo\0%x0%x\%s',
      [oLoCharSet, oHiCharSet, Key[nIndex]]);
    if VerQueryValue(poBuffer, PChar(nStrFileInfo), oReturn, UINT(pnLength))
    then
...
procedure TDMFileInfoUtils.SetSize(const Value: Double);
begin
  FSize := Value;
end;
...
begin
  TDMUtilsMessage.ShowMessageOk('Empresa: ' + FCompanyName + #13 + #10 +
    'Descricao: ' + FFileDescription + #13 + #10 + 'Versao do Arquivo: ' +
    FFileVersion + #13 + #10 + 'Nome Interno: ' + FInternalName + #13 + #10 +
    'Copyright: ' + FLegalCopyright + #13 + #10 + 'Nome Original do Arquivo: ' +
    FOriginalFilename + #13 + #10 + 'Produto: ' + FProductName + #13 + #10 +
    'Versao do Produto: ' + FProductVersion + #13 + #10 + 'Comentarios: ' +
    FComments + #13 + #10 + 'Atributos: ' + FAttribute + #13 + #10 + 'Tipo: ' +
    FTypeFile + #13 + #10 + 'OS: ' + FOS + #13 + #10 + 'Diretório: ' +
    FFileName);
end;
