...
uses
  uDMFileInfoUtils, SysUtils, classes, uDMList;
...
begin
  ...
  if (TDMUtils.CompareIgnoreCase(poFileInfoUtils.NameValidate,
    poCompareFileInfoUtils.NameValidate) or TDMUtils.CompareIgnoreCase
    (poFileInfoUtils.NameValidate, poCompareFileInfoUtils.FileName) or
    TDMUtils.CompareIgnoreCase(poFileInfoUtils.FileName,
    poCompareFileInfoUtils.FileName) or LocateItem(poFileInfoUtils.MD5,
    poCompareFileInfoUtils.MD5) or LocateItem(poFileInfoUtils.CompanyName,
    poCompareFileInfoUtils.CompanyName) or
    LocateItem(poFileInfoUtils.FileDescription,
    poCompareFileInfoUtils.FileDescription) or
    LocateItem(poFileInfoUtils.InternalName,
    poCompareFileInfoUtils.InternalName) or
    LocateItem(poFileInfoUtils.LegalCopyright,
    poCompareFileInfoUtils.LegalCopyright) or
    LocateItem(poFileInfoUtils.OriginalFilename,
    poCompareFileInfoUtils.OriginalFilename) or
    LocateItem(poFileInfoUtils.ProductName, poCompareFileInfoUtils.ProductName)
    or LocateItem(poFileInfoUtils.FileName, poCompareFileInfoUtils.FileName) or
    LocateItem(ExtractFilePath(poFileInfoUtils.FileName),
    ExtractFilePath(poCompareFileInfoUtils.FileName)) or
    LocateItem(ExtractFileName(poFileInfoUtils.FileName),
    ExtractFileName(poCompareFileInfoUtils.FileName)) or
    LocateItem(poFileInfoUtils.NameValidate,
    poCompareFileInfoUtils.NameValidate)) then
    Result := True;
end;
